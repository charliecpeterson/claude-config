...
For **audio-to-video** on LTX-2 (audio file animates a still or generates from text):
...
**Audio cue (if generating audio too):**
\`\`\`
...
```
blurry, low quality, distorted, watermark, signature, text artifacts, deformed, malformed limbs, extra fingers, poorly drawn hands, poorly drawn faces, jittery motion, flickering, frame interpolation artifacts, motion smearing, oversaturated, washed out
```
...
### Example 5 — improving an existing prompt
