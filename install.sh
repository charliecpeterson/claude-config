...
# Codex (and other AGENTS.md agents) can't resolve the @imports in CLAUDE.md,
# so flatten the four rule files into ~/.codex/AGENTS.md. Regenerated each run;
# a pre-existing hand-written file is backed up once.
generate_codex_agents_md() {
  ...
  echo "  write    $dest (global rules for Codex)"
}
...
run_check() {
  ...
  if [[ -f "$CODEX_DIR/AGENTS.md" ]]; then
    ...
    echo "  ✗ ~/.codex/AGENTS.md missing (re-run ./install.sh)"
  fi
